<?php
	...
	function getsetting($pid,$stype) {
		$quer = "SELECT settingvalue FROM roblox_settings_$pid WHERE settingname='$stype'";
		//echo $quer;
		$r = mysql_query($quer);
		echo mysql_error();
		if (!$r) die();
		if (!mysql_num_rows($r)) die("Setting name $stype invalid");
	...
	function getsettings($pid) {
		return [
			...
			"FUN" 							=> stringToBoolean(getsetting($pid, "fun")),
			...
			"GivePlayerPointsFromPurchases"	=> stringToBoolean(getsetting($pid, "ppfrompurchase")),
			"LinkedAccount"					=> getsetting($pid, "linkacc"),
		...
		$bdgid) 
	{
		...
		setsetting($pid,"ppfrompurhcase",$ppfrompurhcase);
		setsetting($pid,"linkacc",$linkacc);
